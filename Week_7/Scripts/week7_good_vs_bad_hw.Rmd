...
grosses_2000 <- grosses %>%
  ...
  group_by(month, year, show)
view(grosses_2000)

ham_week <- grosses_2000 %>%
  filter(show == "Hamilton") %>%
  select(show, month, year, weekly_gross, potential_gross)
view(ham_week)
```
...
grosses_2000 %>%
  ggplot(aes(x = year), color = show) +
  geom_line(aes(y=weekly_gross_overall, col = show)) +
  scale_y_continuous(labels = scales::comma) +
  labs(title = "here we go again!",
       subtitle = "let's have a celebration the glinda",
       x = "i live another 50 years, it's not enough",
       y = "money, money, money",
...
This is an example of a bad plot. There are a few reasons why this is considered a good plot:
  ...
  - The colors used are visible and are easy to read.
  - 
  
```{r, goodplot, outwidth = "50%", fig.align='center'}
ggplot(data = ham_week, mapping = aes(x=year)) +
  geom_line(aes(y = weekly_gross, col = "Weekly Gross")) +
  geom_line(aes(y = potential_gross, col = "Potential Gross")) +
  labs(title = "Weekly Ticket Sales For Hamilton: An American Musical",
       x = "Year",
       y = "Gross in Dollars",
       caption = "Source: The Broadway League",
       colour = "Type of Gross") +
  scale_color_manual(values = pokepal(31)) +
